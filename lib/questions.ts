...
const SECTIONS = {
  ...
  shortAnswer: "四、简答题（每题 8 分，共 16 分）",
  scenario: "五、场景分析题（共 18 分）",
} as const;
...
export const questions: Question[] = [
  ...
  scQ(
    ...
    18,
    [
      { id: "sc1a", text: "(a) 请设计至少 3 个 Agent 角色，说明每个 Agent 的职责。（5分）", maxScore: 5 },
      { id: "sc1b", text: "(b) 画出这些 Agent 之间的协作流程（文字描述即可），说明消息传递的顺序和关键节点。（7分）", maxScore: 7 },
      { id: "sc1c", text: `(c) 如果数据采集 Agent 获取到的数据格式不一致（部分为 JSON、部分为 HTML），你会如何在 Agent 协作流程中处理这个问题？请结合 GlueCoding 的\u300c来源\u2192用途\u2192留存\u2192合规\u300d思路回答。（6分）`, maxScore: 6 },
    ]
  ),
];
